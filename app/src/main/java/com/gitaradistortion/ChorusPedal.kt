...
class ChorusPedal {
    ...
    var onDepthChanged: ((Float) -> Unit)? = null
    ...
    fun makeView(ctx: Context): LinearLayout {
        ...
        card.setBackgroundColor(0xFF102040.toInt())
        card.minimumWidth = 170

        val title = TextView(ctx)
        title.text = "🫧 CHORUS"
        title.setTextColor(0xFF66AAFF.toInt())
        title.textSize = 13f
        ...
        btn.setOnClickListener {
            ...
            btn.setBackgroundColor(if(isEnabled)0xFF2266DD.toInt() else 0xFF444444.toInt())
        ...
        val k1 = makeKnob(ctx,"SPEED",speed,0xFF66AAFF.toInt()){speed=it;onSpeedChanged?.invoke(it)}
        val k2 = makeKnob(ctx,"DEPTH",depth,0xFF88CCFF.toInt()){depth=it;onDepthChanged?.invoke(it)}
