...
class AmpPedal {
    ...
    var onResponseChanged: ((Float) -> Unit)? = null
    ...
    fun makeView(ctx: Context): LinearLayout {
        ...
        card.setBackgroundColor(0xFF251500.toInt())
        card.minimumWidth = 170

        val title = TextView(ctx)
        title.text = "🔊 AMP"
        title.setTextColor(0xFFFFBB44.toInt())
        title.textSize = 13f
        ...
        btn.setOnClickListener {
            ...
            btn.setBackgroundColor(if (isEnabled) 0xFFFF9900.toInt() else 0xFF444444.toInt())
        ...
        val k1 = makeKnob(ctx,"GAIN",gain,0xFFFFBB44.toInt()){gain=it;onGainChanged?.invoke(it)}
        val k2 = makeKnob(ctx,"RESPONSE",response,0xFFFFDD88.toInt()){response=it;onResponseChanged?.invoke(it)}
