...
object AudioMixer {
    ...
    fun applyPreset(name:String) {
        when(name) {
            "Clean" -> {
                ...
                chorus=0.15f; flanger=0.0f; phaser=0.0f; tremolo=0.0f; vibrato=0.0f
            ...
            "Blues" -> {
                ...
                chorus=0.35f; flanger=0.1f; phaser=0.15f; tremolo=0.1f; vibrato=0.15f
            ...
            "Rock" -> {
                ...
                chorus=0.2f; flanger=0.25f; phaser=0.3f; tremolo=0.15f; vibrato=0.0f
            ...
            "Metal" -> {
                ...
                chorus=0.0f; flanger=0.15f; phaser=0.4f; tremolo=0.0f; vibrato=0.0f
    ...
    fun savePreset(ctx: android.content.Context, name:String) {
        ctx.getSharedPreferences("GitaraPresets",0).edit().apply {
            ...
            putFloat("${name}_chorus",chorus); putFloat("${name}_flange",flanger)
            putFloat("${name}_phaser",phaser); putFloat("${name}_trem",tremolo)
            putFloat("${name}_vib",vibrato); putFloat("${name}_delay",delay)
            putFloat("${name}_rev",reverb); putFloat("${name}_wah",wah)
            putFloat("${name}_amp",ampType); putFloat("${name}_bass",bass)
            putFloat("${name}_mid",mid); putFloat("${name}_treble",treble)
        }.apply()
    }
    ...
    fun process(input:Float):Float {
        ...
        if(flanger > 0.05f) {
            flangPhase += flanger * 0.05; if(flangPhase > PI*2) flangPhase -= PI*2
            val mod = (sin(flangPhase) * flanger * 0.3f).toFloat()
            val delayed = sig * (0.3f + mod)
            sig = sig * (1f - flanger*0.3f) + delayed * flanger
        }
