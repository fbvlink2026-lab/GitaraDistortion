...
class PedalBoardFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        hint.text = "📦 Pumunta sa CABINET para makita ang iba pang pedal"
        hint.textSize = 11f
        ...
        val onBoard = listOf(
            "VOLUME" to 0xFFFF8822.toInt(),
            "TONE" to 0xFF44DD88.toInt(),
            "REVERB" to 0xFFAA66FF.toInt(),
            "NOISE GATE" to 0xFF66DDDD.toInt()
        )
        ...
        onBoard.chunked(2).forEach { rowPair ->
            ...
            rowPair.forEach { (name, color) ->
                row.addView(makePedalCard(ctx, name, color))
            }
            grid.addView(row)
        }
        root.addView(grid)
    ...
    private fun makePedalCard(ctx: Context, name: String, themeColor: Int): LinearLayout {
        ...
        btnPower.setOnClickListener {
            ...
            btnPower.setBackgroundColor(if (isOn) themeColor else 0xFF555555.toInt())
        }
